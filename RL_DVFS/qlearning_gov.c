...
#define P_REC "./script/p_record.txt"
#define CPI_REC "./script/cpi_record.txt"
...
float get_cpu_power(void)
{
    ...
    float power_value = (float) power_mW / 1000;
    printf("cpu_power: %f\n", power_value);
    fclose(cpu_power);
...
float get_cpu_volts(void)
{
    ...
    float vol_value = (float) vol_mV / 1000;
    printf("cpu_volts: %f\n", vol_value);
    fclose(cpu_volts);
...
float get_cpu_temp(void)
{
    ...
    float temp_value = (float) tmp / 1000;
    printf("cpu_temp: %f\n", temp_value);
    fclose(cpu_temp);
...
float *get_raw_state(float *state_buff)
{
    printf("\n----------------\n");
                         ...
                         float reward)
{
    ...
    Q[old_s_index][last_action_index] =
        old_return + ALPHA * (total_return - old_return);
    //}
}
...
void run(void)
{
    if (!fopen(Q_EXP, "r")) {
        ...
        FILE *power = fopen(P_REC, "w");
        FILE *cpi_record = fopen(CPI_REC, "w");
        float *state_buff = (float *) malloc(sizeof(float) * NUM_STATES);
        ...
        while (episode < MAX_EPISODE) {
            ...
            /*
                    // Penalize trying to go out of bounds, since there is no
               utility in doing so. if (best_action_index < 0 ||
               best_action_index > NUM_ACTIONS) { if (best_action_index < 0) {
                            reward = reward - (500 * (NUM_ACTIONS -
               best_action_index));
                        }
                        reward = reward - (500 * (best_action_index -
               NUM_ACTIONS));
                    }
            */
            /*
