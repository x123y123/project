...
//read specify pid----------------------------
    ...
    FILE *command = popen("pidof mortor_control","r");
    //FILE *command = popen("pidof video-viewer","r");
...
//--------------------------------------------------------
    ...
    time_t endwait = time(NULL) + test_sec;
    ...
    //读取最新的计数值，每次读取一个结构体
        ...
        ipc = (float)aread.values[1]/(float)aread.values[0];
        ioctl(fd,PERF_EVENT_IOC_RESET,PERF_IOC_FLAG_GROUP);
   
        
        //printf("pid_id: %d\n", pid_id);
        printf("\ncpu_cycles = %ld\ninstructions = %ld\nIPC = %f\n",aread.values[0],aread.values[1], ipc);
        sleep(1);
        //usleep(500000);
    }
}
